...
flowchart LR
    ...
    subgraph Docker Compose 栈
        ...
        MCP[mcp: :3333/mcp<br/>51 工具 Streamable HTTP]
...
- `tests/test_stdio_and_redact.py`（8 项）：stdio 协议往返、51 工具数断言、日志脱敏
- 运行：`python -m pytest tests/ -q`（3 项 skill-inventory 用例需要 git 仓库环境，属环境性跳过）

## 7. P3 运行环境检查

`backend/core/environment.py` 不涉及平台网络访问，只验证端口占用、数据目录写权限、FFmpeg 和磁盘空间。结果通过 `/api/environment` 暴露给 Web，并在仪表盘展示；Agent 通过 MCP `environment_check` 获取相同结果。这样可以在提交采集前区分“服务可用”“可选能力降级”和“环境阻塞”，而不是把所有问题归为平台故障。
